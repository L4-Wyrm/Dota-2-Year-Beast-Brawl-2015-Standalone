...
function shootEnd2(keys)
  ...
  projCollider.radius = 175
  projCollider.test = function(self, collider, collided)
    return IsPhysicsUnit(collided) and collided.IsRealHero and collided:IsRealHero() and collided ~= caster and collided.IsAlive and collided:IsAlive()
  end
  projCollider.preaction = function(self, collider, collided)
    print("damageandpush")
    collided:EmitSound("Hero_Lich.ChainFrostImpact.Hero")
    damageAndPush(caster, collided, 100 * chargeTime / 1.8, 250 * chargeTime / 1.8, collider.vVelocity:Normalized())
  end
